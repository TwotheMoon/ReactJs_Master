...
import { getPopularTV, getSimilarTV, getTV, IGetMoviesResult } from "../api";
...
const rowVariants2 = {
    hidden2: (back: boolean) => ({
        x: back ? -window.innerWidth - 5 : window.innerWidth + 5,
    }),
    visible2: {
        x: 0,
    },
    exiting2: (back: boolean) => ({
        x: back ? window.innerWidth + 5 : -window.innerWidth - 5,
    }),
};
const BoxVariants = {
...
function Tv() {
    ...
    const { data: popularData } = useQuery<IGetMoviesResult>(["popualrMovies", "popular"], getPopularTV);
    ...
    const increaseIndex = () => {
        if (data) { // 만약 data가 없을때 오류 방지를 위해
            ...
            setBack(false);
            toggleLeaving();
            const totalTvs = data?.results.length - 1; // 메인 영화 1개 빼고
            const maxIndex = Math.floor(totalTvs / offset) - 1; // 실수가 나올 수 도 있으니 무조건 내림 처리 (추가 1 ~ 2개 영화 안보이기)
    ...
    const increaseIndex2 = () => {
        if (popularData) { // 만약 data가 없을때 오류 방지를 위해
            if (leaving2) return;
            setBack2(false);
            toggleLeaving2();
            const totalTvs = popularData?.results.length - 1; // 메인 영화 1개 빼고
            const maxIndex = Math.floor(totalTvs / offset) - 1; // 실수가 나올 수 도 있으니 무조건 내림 처리 (추가 1 ~ 2개 영화 안보이기)
    ...
    const clickedMovie2 = bigMovieMatch?.params.tvId &&
        popularData?.results.find((tv) => String(tv.id) === bigMovieMatch.params.tvId);
    return (
        <Wrapper>{isLoading ? (
            ...
            <>
                ...
                <Slider onClick={() => { setClickSlider(2) }}>
                    <h1>영화가 드라마로 나왔다고? 얼른 보자! </h1>
                    <AnimatePresence custom={back2} initial={false} onExitComplete={toggleLeaving2}>
                        <SliderBtn onClick={decreaseIndex2}><i className="fas fa-chevron-left fa-2x"></i></SliderBtn>
                        <Row
                    ...
                    <SliderBtn onClick={increaseIndex2}><i className="fas fa-chevron-right fa-2x"></i></SliderBtn>
                </Slider>
                <AnimatePresence>
                    {bigMovieMatch && clickSlider === 2 ?
                        <>
                            <Overlay
                                onClick={onOverlayClick}
                                animate={{ opacity: 1 }}
                            ...
                            >
                                {clickedMovie2 && <>
                                    <BigCover style={{ backgroundImage: `linear-gradient(to top, black, transparent), url(${makeImagePath(clickedMovie2.backdrop_path, "w500")})` }} />
                                    <IconWrap>
                                        <Icons>
                                            <IconCircle>
                                                <i className="fas fa-plus"></i>
                                        ...
                                        <BigInfo>
                                            <span>첫방송:</span> {clickedMovie2.first_air_date} <br />
                                            <span>평점:</span> {clickedMovie2.vote_average}
                                        </BigInfo>
                                    </IconWrap>
                                    <BigTitle>{clickedMovie2.name}</BigTitle>
